...
def get_chrome_instance():
    chromedriver_path = r"C:\Users\27925\.cache\selenium\chromedriver\win64\143.0.7499.42\chromedriver.exe"
...
def search_in_baidu(query: str) -> str:
    ...
    try:
        ...
        #渲染需要时间，最多等 10 秒，直到页面上出现 id="chat-textarea" 的元素
        ...
        #最多等 5 秒，直到浏览器标签页的标题包含你搜索关键词的前 10 个字符;
        WebDriverWait(driver, 5).until(EC.title_contains(query[:10]))
            ...
            if i > 0:
                results = WebDriverWait(driver, 5).until(
                    EC.presence_of_all_elements_located(
                        (By.CSS_SELECTOR, ".page-inner_2jZi2>a")))
                if i == 1:
                    results[i].click()
                else:
                    results[i].click()
...
def pretty_html(html: str) -> str:
    ...
    for tag in soup(
        ["script", "style", "link", "meta", "symbol", "path", "canvas",
         "svg"]):
        tag.extract()
    ...
    for tag in soup.find_all(True):
        if tag.name == "a":
            if "href" in tag.attrs:
                if "javascript:" in tag.attrs["href"] or "/" == tag.attrs[
                        "href"]:
                    tag.extract()
                else:
