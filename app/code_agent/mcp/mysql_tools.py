...
def mysql_describe_tables(database: str, table: str):
    try:
        result, rowcount = execute_query(f"describe {table}",
                                         database=database)
        return Response(
            success=True,
            database=database,
...
def mysql_insert_data(database: str, table: str, data: Dict[str, str]):
    ...
    #输出格式是例如["%s", "%s", "%s"] ["value1", "value2", "value3"]
    command = f"INSERT INTO {table} ({','.join(columns)}) VALUES ({values_wrapper})"

    try:
        # 执行插入操作，提交事务;commit=True将自动提交事务
        result, rowcount = execute_query(command,
                                         database=database,
                                         params=tuple(values),
                                         commit=True)
...
def mysql_update_data(database: str, table: str, data: Dict[str, str],
                      where: Dict[str, str]):
...
def mysql_create_table(database: str, table_name: str, table_columns: Annotated[
    str,
    Field(
...
if __name__ == '__main__':
    ...
    #     mysql_create_table(
    #         "test1", "menu",
    #         "`id` int NOT NULL AUTO_INCREMENT, `name` varchar(255) NOT NULL, `price` decimal(10, 2), PRIMARY KEY (`id`)",
